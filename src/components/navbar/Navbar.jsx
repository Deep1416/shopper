      ...
      <header className="box ">
        <nav className="flex justify-between items-center w-full h-full">
          <div className="flex items-center gap-2 md:gap-[10px]">
            ...
            <p className="text-[#383838] cursor-pointer text-[16px] md:text-[24px] font-bold">
              SHOPPER
            </p>
          </div>
          <div>
          ...
          <div className="hidden md:flex items-center justify-center gap-5">
            {userRole === "user" ? (
              <button
                onClick={logOutClear}
                className="bg-[#ff4141] py-1 px-6 rounded-md text-lg font-medium"
              >
                LogOut
              </button>
            ) : (
              <Link to={"/signup"}>
                <button className="bg-[#ff4141] py-1 px-6 rounded-md text-lg font-medium">
                  LogIn
                </button>
              </Link>
            )}
            ...
            <div className="relative text-white ">
              <Link to={"/cart"}>
                {cartSlice.length === 0 ? null : (
                  <span className="absolute bottom-5 right-1 text-lg w-4 h-4 rounded-full text-yellow-300 flex justify-center items-center bg-[#ff4141]">
                    {cartSlice.length}
                  </span>
                )}
